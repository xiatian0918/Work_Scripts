...
a=$(curl -ksX PATCH --header "Content-Type: application/json" --header "Accept: application/json" --header "vmware-api-session-id:"$session2""  -d '{"spec": { "hot_add_enabled": false, "size_MiB": "'"$core"'" }}' "https://*******/rest/vcenter/vm/"$vm_name"/hardware/memory"|jq .)
