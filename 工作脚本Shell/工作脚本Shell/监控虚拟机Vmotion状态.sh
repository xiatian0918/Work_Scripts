...
vm_mac=$(curl -ksX GET --header 'Accept: application/json' --header "vmware-api-session-id: $session2" "https://*******/rest/vcenter/vm/${vm_mm2}/hardware/ethernet/${vm_nic2}"|jq .|grep mac_address|awk -F\": '{print $2}'|awk -F\, '{print $1}'|awk -F\" '{print $2}')

echo $vm_mac >/tmp/mac.txt

scp -r /tmp/mac.txt root@*******.64:/tmp/
