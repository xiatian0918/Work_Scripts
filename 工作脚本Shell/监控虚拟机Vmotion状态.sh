...
vm_mac=$(curl -ksX GET --header 'Accept: application/json' --header "vmware-api-session-id: $session2" "https://192.168.0.24/rest/vcenter/vm/${vm_mm2}/hardware/ethernet/${vm_nic2}"|jq .|grep mac_address|awk -F\": '{print $2}'|awk -F\, '{print $1}'|awk -F\" '{print $2}')

echo $vm_mac >/tmp/mac.txt

scp -r /tmp/mac.txt root@192.168.0.64:/tmp/
